...
{	
	...
	*/
	...
	UCO_Misc_AdditionalExp,		// 안씀
	UCO_Misc_AdditionalGold,	// 안씀
...
struct FItemOption
{
	...
	/** Default 값은  EOptionVariation::EOV_Incerease 이며, 능력치가 감소하는 특별한 상황에서만 접근한다. */
...
/**
* 이 옵션은 감소 능력치 적용을 위해 추가되었으며, 기존에 능력치들은 합산을 TArray(float) 를 가지고 순차적으로 합산해가며 계산했으나
* 감소해야 할 경우에는 반대로 계산해주기 계산용으로 추가
*/
struct FOptionValue
{
	FOptionValue()
