...
void UB2UIMissionList::OnReceiveMissionReward(const FB2ReceiveMissionReward& MissionReward)
{
	...
	//	}
	//	// 傈眉 焊惑罐扁甫 喘范促搁 葛电 焊惑捞 肯丰瞪 锭 鳖瘤 UI 盎脚窍瘤 臼澜
	//	else
	//	{
	...
	//			// 老馆烙公 傈何促罐扁甫 柳青 吝老 版快 格废 盎脚阑 窍咯 犁焊惑阑 罐阑霸 乐绰瘤 促矫 犬牢
...
void UB2UIMissionList::OnReceiveMissionPointReward(const FB2ReceiveCompleteMissionReward& CompleteReward)
{
	...
	//if (bRequestRewardAllCompleteMission && RewardPointIndex > 0)				//葛滴罐扁窍绊 焊惑捞 巢酒乐阑版快
	//{
	//	if (GetWorld())
	//	{
	//		ResponseTime = GetWorld()->GetTimeSeconds();
...
{
	SetAllRewardButtonDisable(false); // 阜篮 焊惑罐扁 滚瓢阑 钱绢
...
void UB2UIMissionList::EndTimerReceiveMissionReward()
{
	...
	{
		int32 PointRewardIndex = CheckMissionPointReward(SelectedMissionType);															//茄锅歹 八荤
