...
{
	int32 SlotNumberRevision = TeamNumber == 2 ? 4 : 0;			//슬롯번호 보정위해서 
...
{
	if (SelectType == EGuildEntrySelectType::Normal)			//클릭했을때 일반상태면 클릭한 상태로 바꿔준다
	{
		SelectType = EGuildEntrySelectType::Select;
		SelectPlaySound();
		SlotDelegate.ExecuteIfBound(EGuildEntrySlotType::FormationSlot, SelectType, GuildEntryCode, SlotNumber);					//슬롯들 갱신해준다
		return;
	}
	else if(SelectType == EGuildEntrySelectType::SelectImpossible || SelectType == EGuildEntrySelectType::Select)	//클릭이 불가능한상태나, 이미클릭한상태에서 똑같은 슬롯에 들어오면 다시 Normal상태로 바꿔준다
...
void UB2UIGuildFormationMercenarySlot::OnClickSlot()
{
	...
	{
		SelectType = EGuildEntrySelectType::Select;
		SlotDelegate.ExecuteIfBound(EGuildEntrySlotType::MercenaryList, SelectType, GuildEntryCode, 0);					//슬롯들 갱신해준다
		SelectPlaySound();
		return;
	}
	else if (SelectType == EGuildEntrySelectType::SelectImpossible || SelectType == EGuildEntrySelectType::Select)	//클릭이 불가능한상태나, 이미클릭한상태에서 똑같은 슬롯에 들어오면 다시 Normal상태로 바꿔준다
