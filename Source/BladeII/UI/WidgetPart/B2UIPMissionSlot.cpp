...
// Master Data 父栏肺档 Setting 啊瓷 茄 巴甸
...
// Mission狼 惑怕甫 Server俊辑 罐酒具父 Update 啊瓷 茄 巴甸
...
void UB2UIPMissionSlot::UpdateStaticText_TitleDesc(MissionPtr Mission)
{
	if (Mission)
	{
		...
		{
			// GoalCount啊 酒囱 condition捞 函版等 楷加固记捞扼搁 Title(MissionID_Step) 苞 鞍篮 器杆狼 虐蔼阑 爱绰促. - Desc悼老
		...
		{
			/*窍靛内爹 13008 ~ 13010 何盒篮  Condition1蔼阑 捞侩秦具茄促.. 酒流篮 迭洒 规过捞 绝嚼聪促*/
...
void UB2UIPMissionSlot::OnClickGetRewardButton()
{
	...
	if (CurrentMission != nullptr)
	{
		...
		{
			SetAllRewardButtonDisableClass<const bool>::GetInstance().Signal(true); // Receive 罐扁傈鳖瘤 Reward 给穿福霸 阜磊
