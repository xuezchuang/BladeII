...
void UB2UIEnterDungeonSlot::CacheAssets()
{
	...
	// Slot BG 는 UMG 내부 BGImage 변수에 등록해서, Construct 에서 세팅하게 되어있음.
	// RewardIcon, RewardName, DungeonName, DungeonDesc, STB_ClosePossible 포함
}
...
{
	if (TB_DungeonCount.IsValid())// (던전 입장횟수 / MAX)
	{
		if (MaxCount > 0)
