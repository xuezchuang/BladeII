...
void UB2UISelectiveLotteryConsumItemPopup::SetItemData(const TArray<int32>& ItemRefIds, const TArray<int32>& ItemNums)
{
	...
	bool bCheck = true;			// 하드 코딩 고급, 일반 승급석 예외처리

	for (int32 Index = 0; Index < ItemRefIds.Num(); Index++)
	{
		if (ItemRefIds[Index] == FItemRefIDHelper::GetRareWeaponUpgradeStoneID() ||		// 고급 무기 승급석
			ItemRefIds[Index] == FItemRefIDHelper::GetRareArmorUpgradeStoneID() ||		// 고급 방어구 승급석
			ItemRefIds[Index] == FItemRefIDHelper::GetRareAccessoryUpgradeStoneID())		// 고급 장신구 승급석
		{
			continue;
		}
		else
