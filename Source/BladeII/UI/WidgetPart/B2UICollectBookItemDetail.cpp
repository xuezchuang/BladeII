...
void UB2UICollectBookItemDetail::SetDetailViewSingleItem(int32 ItemTemplateID, EPCClass ClassType, const int32 SurpassLevel/* = 0*/, FB2Item OriginalItem)
{
	...
	if (ItemInfo)
	{
		...
		MaxLevel = 30 + SurpassLevel;	//扁夯 酒捞袍 弥措 饭骇捞 30捞扼绊 啊沥茄促. 弊 捞惑狼 巴档 辑滚俊辑 弥措饭骇 力茄捞 吧妨辑 箭磊甫 嘎眠扁 困秦 捞犯霸 柳青茄促.
		...
		if (DetailPart)
		{
			...
			// UniqueOption捞 绝栏搁 RandomOption 咆胶飘啊 HeaderTitle阑 措脚 窃
			...
			SetItemPrimaryOption(ItemInfo->PrimaryType, MinQualityPrimaryPoint, MaxQualityPrimaryPoint); //傈捧仿拌魂阑 秦具窍扁锭巩俊 力老 关栏肺 倒府摆嚼聪促
		}
...
void UB2UICollectBookItemDetail::SetSelectiveLottery()
{
	...
	//急琶 惶扁鼻俊辑 饭骇, 碍拳甫 炼例窍绰吧 阜绰 备巩牢单, 扁裙率俊辑 急琶 惶扁鼻俊辑档 单葛肺 焊咯林磊绊 秦辑 林籍贸府 窃.
	/*
	...
	if (TB_Enhance.IsValid())
	{
		//扁粮俊 +0 备巩 磊眉啊 喉风橇赴飘 窍靛内爹捞看促. 弊矾骨肺 咯扁档 弊成 窍靛内爹栏肺 丹绢滚府磊.
