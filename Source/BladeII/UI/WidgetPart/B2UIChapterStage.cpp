...
void UB2UIChapterStage::BindDoc(/*const FStage绢录绊& InStageData*/)
{
	UnbindDoc();
	...
	else
	{
		...
		int32 BeforeChapterNum = ChapterNum - 1;										//捞傈 茅磐 瘤苛鳖瘤 并绰瘤 俺魂
		FServerStageID  LastClearClientStageIdHell = StageDataStore.GetLastClearServerStageId(EStageDifficulty::ESD_Hell);					//付瘤阜 秉 serverStageID
		FServerStageID	ServerStageLastNum = StageDataStore.GetServerStageID(BeforeChapterNum * 10, EStageDifficulty::ESD_Hell);			//捞傈茅磐 Hell付瘤阜 胶抛捞瘤 酒捞叼
