...
#include "B2UIEventSlotGemSpend.generated.h"

/**
 * 
 */
UCLASS()
class BLADEII_API UB2UIEventSlotGemSpend : public UB2UIEventSlot
{
	GENERATED_BODY()
...
private:
	...
	void InitRewardIconList(const b2network::B2mdSpendGemEvent& EventRewardInfo);

public:
	void InitEventSlotGemSpend(const b2network::B2mdSpendGemEvent& EventRewardInfo);
	virtual void UpdateVisibleState() override;
	void SetGemProgressAndGoalCountText(const int32 GemCount);
	int32 GetGoalCount() { return ChachedEventRewardInfo.goal_spend_count; }
...
private:
	b2network::B2mdSpendGemEvent			ChachedEventRewardInfo;
};
