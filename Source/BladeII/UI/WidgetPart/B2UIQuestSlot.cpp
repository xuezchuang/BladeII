...
void UB2UIQuestSlot::MainQuestActFilter(int32 InQuestID)
{
	...
	if (MainQuestMasterData->main_view_condition == 0)  //main_view_condition ==0老锭绰 芭弗促
		return;

	FStageDataStore& StageStore = BladeIIGameImpl::GetStageDataStore();
	FServerStageID LastServerStageID;

	for (int32 i = 0; i < static_cast<int32>(EStageDifficulty::ESD_End); ++i)								//啊厘奴 ServerStageID甫 茫绰促 畴富 Act6篮 5-10瘤苛焊促 StageID啊 歹 农促
	{
