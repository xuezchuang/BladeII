...
void UB2UIFriendKakaoInvite::CreateInviteFriendList()
{
	...
	// 2000疙捞惑 模备格废阑 佬绢焊搁 SDK惑俊辑 肋给等 模备 格废阑 府畔秦辑
	// 墨墨坷螟苞 蛆狼 饶 2000捞惑篮 救焊捞霸 荐沥窃..
...
void UB2UIFriendKakaoInvite::OnKakaoInvitePlatformCallback(const int32 ResultCode)
{
	...
	else
	{
		...
		switch (Code)
		{
		case EKakaoInviteResultCode::ExceedMonthlyUsage:	// 岿埃 倾侩 荤侩樊 檬苞
			ErrorText = FString(TEXT("Friend_KakaoFriendInviteCondition2"));
			break;
		case EKakaoInviteResultCode::ExceedDailyUsage:		// 老埃 倾侩 荤侩樊 檬苞
			ErrorText = FString(TEXT("Friend_KakaoFriendInviteExceed"));
			break;
		case EKakaoInviteResultCode::SettingDisabled:		// 荐脚 瞒窜 惑怕
			ErrorText = FString(TEXT("Friend_KakaoFriendInviteDenied"));
			break;
// 		case EKakaoInviteResultCode::NotKakaoTalkUser:		// 墨墨坷佩 蜡历 酒丛 (捞扒救靖)
