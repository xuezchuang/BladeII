...
void UB2LobbyUI_RelicManagement::OnCreateRelicBoard()
{
	...
	// TSoftObjectPtr 狼 窜鉴 LoadSynchronous 绰 Async flush 甫 老栏懦 荐 乐瘤父 捞吧 荤侩且 鸥捞怪俊 async flush 甫 捌沥且 鞘夸绰 绝阑 巴 鞍栏骨肺 弊成 荤侩.
...
void UB2LobbyUI_RelicManagement::RequestEnchanceRelic()
{
	...
	if (LevelInfo)
	{
		...
		if (BladeIIGameImpl::GetClientDataStore().GetGoldAmount() < LevelInfo->nEnhance_Gold) // 榜靛何练
		{
			bIsAuto = false;
			bIsAni = false;
			CheckButtonEnable();
...
void UB2LobbyUI_RelicManagement::RequestPromotionRelic()
{
	...
	if (GradeInfo)
	{
		...
		if (BladeIIGameImpl::GetClientDataStore().GetGoldAmount() < GradeInfo->nPromotion_Gold)
		{
			...
			bIsAni = false;
			CheckButtonEnable();
			ShortageMGR->PopupGoToShopForGoldMessage(); // 榜靛何练
			return;
		}
