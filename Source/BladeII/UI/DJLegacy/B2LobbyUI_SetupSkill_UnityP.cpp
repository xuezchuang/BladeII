...
void UB2LobbyUI_SetupSkill_UnityP::InitFromOwnerSkillMain(UB2LobbyUI_SetupSkillMain* InOwnerPage, UB2LobbyUI_HeroMgmtCommon* InHeroMgmtCommon)
{
	...
	HeroMgmtBaseNRef = InHeroMgmtCommon; // 전달만 받고 이쪽서 Init 같은 거 안함.
...
{
	if (InPCClassIfChanged != EPCClass::EPC_End) // 필요한 경우 PCClass 이걸로 통해 변경.
	{
		MainPCClass = InPCClassIfChanged;
	}

	// 이전에 SetupSkillMain 이랑 붙어있을 때 이쪽 탭 선택이나 영웅관리 클래스 교체 시 수행되던 동작 모음.
	SetUnitySkillPanel();
...
{
	int32 CurrentUnitySkillIndex = GetUnitySkillID(MainPCClass); // [todo] 이거 로컬 케릭터데이터로 해야될듯?
...
void UB2LobbyUI_SetupSkill_UnityP::OnClickedUnityPortraitButton(EPCClass seletedclass)
{
	...
	for (auto MissionItem : *CurrentMissions) 
	{
		if (MissionItem.SubClass == SubPCClass)
		{
			...
			for (int32 i = 1; i < 3; i++) // 각 미션의 조건인덱스는 1,2
			{
				...
				if (MissionInfoWidget)
				{
					...
					if (!CheckCanReward) // 하나라도 조건에 맞지 않을경우 보상받을수없음
						bRewardAble = false;
				}
			}
		}
	...
	if (IMG_UnitySelect.IsValid()) // 중앙버튼의 선택 이미지 처리는 따로.
