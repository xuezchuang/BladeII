...
void UB2WingUIP_PropOptionDetail::UpdateWidgets()
{
	...
	// bAtMaxLevel 捞搁 酒付档 喊促弗 蔼阑 掘绢坷瘤 给且 淀.
	...
	// OV_EnhanceBtnSet 苞 OV_AtMaxLevelSet 篮 WS_PerOptionStateSet 狼 老何啊 瞪 荐档..
	...
	{// 酒付 OV_EnhanceBtnSet 狼 老何摆瘤父 犬角洒..
		...
		{ // 窜瘤 犁丰 葛磊扼辑 碍拳 给窍绰 惑炔捞搁 disable.. 捞 酒聪绊 泅龙蜡档 扑诀阑 剁匡 巴.
	...
	else
	{
		...
		if (TB_RequiredFeatherNum.IsValid())
		{
			...
			// 犁丰 葛磊鄂 版快 弧埃祸栏肺 窍绰单 老窜 祸惑篮 环秦 焊捞聪 窍靛内爹..
...
{
	// CachedWingData 客 CachedOptionIndex 绰 诀单捞飘 等 惑怕咯具
	...
	return (OutCurrentFeatherNum >= OutRequiredFeatherNum); // 付瘤阜栏肺 啊瓷 咯何 府畔
}
...
void UB2WingUIP_PropOptionDetail::OnClickedEnhanceBtn()
{
	if (!UB2Airport::HasEnoughGoldForWingOptionEnhanceLocalPlayer(IntToPCClass(CachedWingData.OwnerPCClass), CachedOptionIndex)){
		// 加己 可记 碍拳俊 捣捞 鞘夸绝绰 茄 咯扁肺 棵 老篮 绝阑 巴..
