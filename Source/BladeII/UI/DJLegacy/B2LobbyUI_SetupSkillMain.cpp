...
void UB2LobbyUI_SetupSkillMain::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	...
	// 咯扁 Unity 颇飘甫 蝶肺 备己沁绰单 弊率 徘 柳涝矫 己瓷捞 冻绢瘤聪 捞 矫痢何磐 肺爹矫累 ぇぇ.
	// Init 矫痢俊绰 async flush 夸家啊 乐栏骨肺 阂啊.
...
void UB2LobbyUI_SetupSkillMain::OnClickedActiveSkillListBtn()
{
	...
	SetUnityTabPartOpen(false); // 菊栏肺 官拆 备己.. UnitySkill 率 喊档 颇飘..
...
void UB2LobbyUI_SetupSkillMain::OnClickedUnitySkillBtn()
{
	if (CurrentView != EViewSkillList::UnitySkill)
	{
		...
		SetUnityTabPartOpen(true); // 菊栏肺 官拆 备己.. UnitySkill 率 喊档 颇飘..
		CreateSkillList();
	}
...
{
	// 胶懦 器牢飘父 抗寇 贸府 (榜靛绰 HandleServerError911Class 辑滚 俊矾肺 贸府)
	if (CurrentSkillPoint < 1) // 胶懦 器牢飘啊 窍唱档 绝阑版快 抗寇贸府..
	{
...
void UB2LobbyUI_SetupSkillMain::CreateSkillList()
{
	...
	TArray<int32> PassiveIndexArray; // 绢恫 浇吩捞 后浇吩捞 登具窍绰瘤 舅扁困秦
	...
	for (int32 i = 0; i < SkillInfoArray.Num() && i < PCSkills.Num(); ++i)
	{
		...
		if (CurrentView == EViewSkillList::PassiveSkill)
		{
			...
			BII_CHECK(PassiveIndex); // EPassiveType 棺 SkillInfoIndexFromPassive俊 菩矫宏 眠啊灯绰瘤 犬牢
			if (PassiveIndex)
			{
	...
	// 浇吩篮 MAX_SKILLINFO_COUNT俺牢单 胶懦捞 MAX_SKILLINFO_COUNT俺 固父老锭 后浇吩 贸府
