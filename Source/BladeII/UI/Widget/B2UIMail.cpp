...
)
{
	...
	if (RewardPopup != nullptr)
	{
		...
		// RewardPopup俊辑 ClientDataStore::AccountInfo俊 焊惑 碍力 利侩
	...
	{
		//康去八 墨抛绊府俊辑绰 皋老ID啊 逞绢坷瘤 臼澜 -> 函版瞪 咯瘤 乐澜.
		VB_MailList->ClearChildren();
...
void UB2UIMail::OpenMailUI(int64 InMailId, const FB2ResponseOpenMailPtr& InMailInfo)
{
	...
	{
		CreatedGeneralLotteryPopup->Close(); // 见扁扁父 窍绊 积己等 芭 颇鲍窍绰 扒 酒丛.
	}
...
//
// 酒贰.. 窍困 UIP 甸狼 dynamic on-demand 肺爹 棺 积己 内靛甸 捞绢咙.
//

void UB2UIMail::OpenGeneralLottery(int64 MailId, const FText& TitleText)
{
	ConditionalCreateGeneralLotteryPopup(); // 悼利 肺爹 棺 积己
...
{
	ConditionalCreateAllSelectiveLotteryPopup(); // 悼利 肺爹 棺 积己
...
{
	ConditionalCreateAllSelectiveLotteryCostumePopup(); // 悼利 肺爹 棺 积己
...
{
	ConditionalCreateSelectiveLotteryPopup(); // 悼利 肺爹 棺 积己
...
{
	ConditionalCreateSelectiveLotteryConsumeItemPopup(); // 悼利 肺爹 棺 积己
...
{
	ConditionalCreateSelectiveLotteryConsumeItemResultPopup(); // 悼利 肺爹 棺 积己
...
{
	ConditionalCreateSelectiveLotteryResultPopup(); // 悼利 肺爹 棺 积己
...
{
	// B2UIMail俊辑 ClientDataStore::AccountInfo俊 焊惑 碍力 利侩
	...
	if (MailTabs[ConvertMailIndexToInt32(CurrentTabIndex)].Mails.Num() > 0)
	{
		for (int32 Idx = MailTabs[ConvertMailIndexToInt32(CurrentTabIndex)].Mails.Num() - 1; Idx > -1; --Idx)
		{
			if (MailTabs[ConvertMailIndexToInt32(CurrentTabIndex)].Mails[Idx]->GetGiftType() == MailGiftType::Selective_Item_Lottery &&
				MailTabs[ConvertMailIndexToInt32(CurrentTabIndex)].Mails[Idx]->GetItemRefId() != 8			// 窍靛 内爹 绊鞭 铰鞭籍 惶扁鼻 抗寇
				)
			{
				return true;
			}
