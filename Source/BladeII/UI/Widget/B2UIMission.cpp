...
void UB2UIMission::Init()
{
	...
	if (SomeDoc)
	{
		...
		SomeDoc->SetCompleteMissionType(static_cast<int32>(EMissionType::Daily)); // Daily肺 促矫 檬扁拳
	}

	ChangeTab(InitMissionType, true); // Default Daily肺 檬扁 悸泼 - Mission 肯丰 芒 官肺啊扁肺 吭促搁 SomeDoc劝侩
	RefreshTabs();
}
...
void UB2UIMission::RefreshTabs()
{
	for (TPair<ETabType, FMissionTab>& Tab : Tabs)
	{
		...
		Tab.Value.SetTabNewIcon(bDisplayNew);	// Complete啊 乐促搁 New钎矫
	}
}
...
void UB2UIMission::ChangeTapShortcut(EMissionType type)
{
	...
	{
		ChangeTab(TabType, true); // Default Daily肺 檬扁 悸泼 - Mission 肯丰 芒 官肺啊扁肺 吭促搁 SomeDoc劝侩
		RefreshTabs();
	}
}
