...
void UB2UIGuildBuffWidget::UpdateStaticText()
{
	...
	if (TB_OneBTNBuy.IsValid())
		TB_OneBTNBuy->SetText(BladeIIGetLOCText(B2LOC_CAT_GENERAL, TEXT("Guild_Buff_PurchaseAll")));	//GeneralText작업 필요
}
...
void UB2UIGuildBuffWidget::ResponseUpdate(b2network::B2GuildSkillPtr InSkill, int32  InGold)
{
	...
	//	int32 Hr_ = FPlatformMath::FloorToInt(Secs_ / 3600);							//시
	//	int32 Min_ = FPlatformMath::FloorToInt((Secs_ - (Hr_ * 3600)) / 60);			//분
	//	int32 Sec_ = FPlatformMath::FloorToInt(Secs_ - (Hr_ * 3600) - (Min_ * 60));	//초
...
void UB2UIGuildBuffWidget::OnClickBTN_OneBTNBuy()
{
	...
	//else
	//{
	//	//골드가 부족해도 눌렀을 때 강제로 경고창을 띄우도록 함
	...
	//	int32 Hr = FPlatformMath::FloorToInt(Secs / 3600);							//시
	//	int32 Min = FPlatformMath::FloorToInt((Secs - (Hr * 3600)) / 60);			//분
	//	int32 Sec = FPlatformMath::FloorToInt(Secs - (Hr * 3600) - (Min * 60));	//초
