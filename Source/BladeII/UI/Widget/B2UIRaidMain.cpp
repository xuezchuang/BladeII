...
int32 UB2UIRaidMain::CheckAbleRaidStep(int32 inStep)
{
	...
	if (!IsRematch) // 颇萍盔 促矫窍扁客 檬措窍扁绰 瘤沥秦霖 胶跑 弊措肺.
		return inStep;
	...
	if (RaidData)
	{
		for (FRaidMasterData Ele : *RaidData)
		{
			...
			else if (Ele.RaidStep == MaxDifficultyNumber && MyCombat >= Ele.RecommendCombatPoint) // 郴 傈捧仿捞 MAX窜拌焊促 臭阑版快 MAX窜拌肺 绊沥.
			{
