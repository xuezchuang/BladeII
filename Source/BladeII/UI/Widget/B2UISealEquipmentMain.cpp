...
void UB2UISealEquipmentMain::UpdateCost(int32 InSlotNumber)
{
	...
	for (FSealOption SealOptionInfo : CacheItem.SealOptions)
	{
		if (InSlotNumber - 1 == SealOptionInfo.SealSlotNumber && SealOptionInfo.IsOpened == false)		//傈浇吩捞 摧囚乐绰瘤 焊磊
		{
		...
		if (InSlotNumber == SealOptionInfo.SealSlotNumber)
		{
			if (SealOptionInfo.IsOpened == false)
			{
				...
				if (IsBeforeSlotLock == false)			//弊捞傈浇吩档 摧囚乐阑版快 滚瓢 厚劝己拳 && 咆胶飘 函版
				{
				...
				{
					TB_LockDesc->SetText(BladeIIGetLOCText(B2LOC_CAT_GENERAL, TEXT("SealPopup_SlotUnlock"))); //眠饶唱坷搁 函版
