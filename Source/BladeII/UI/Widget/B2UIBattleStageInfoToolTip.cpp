...
void UB2UIBattleStageInfoToolTip::ShowRewardItemInfo(const FB2RewardItemPreviewInfo& InRewardInfoData)
{
	...
	if (TB_RewardItemDesc.IsValid())
	{
		...
		TempString += FText::AsNumber((InRewardInfoData.SomeCommonData.ItemRefID / 1000) % 100).ToString(); // 角力 粮犁窍绰 酒捞袍捞 酒聪扼, 焊咯林扁侥 酒捞袍捞骨肺 吝埃 牢郸胶 荤侩
...
void UB2UIBattleStageInfoToolTip::ShowBossPieceInfo(const FB2RewardItemPreviewInfo& InRewardInfoData)
{
	...
	////炼阿篮 汲疙捞 甸绢皑. RewardItemInfo俊辑 TB_RewardItemOptionInfo俊 持瘤父 咯扁辑 弊困俊 促矫 丹绢 竞框
	//if (TB_RewardItemDesc.IsValid())
	//{
...
void UB2UIBattleStageInfoToolTip::ShowEtherItemInfo(const FB2RewardItemPreviewInfo& InRewardInfoData)
{
	...
	{
		b2network::B2mdAetherSetOptionInfoPtr SetOptionPtr = B2EtherManager::GetInstance().GetEtherSetOptionInfoPtrByType(InRewardInfoData.SomeCommonData.Quality, 1); //tier绰 1肺 劝侩
