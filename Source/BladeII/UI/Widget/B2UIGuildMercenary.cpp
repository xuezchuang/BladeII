...
void UB2UIGuildMercenary::SetMercenary()
{
	...
	auto NPCData = GuildData.GetGuildAllMercenaryData();				//泅犁 啊瘤绊乐绰 侩捍单捞鸥
	//auto NPCMasterDataArr = GuildData.GetGuildMercenaryMasterData();			//侩捍 沥焊 付胶磐 单捞鸥(傈眉)
	...
	{
		//auto NPCMasterData = NPCMasterDataArr.GetMercenaries(ElNPCData.Value->id);				//侩捍 沥焊 付胶磐 单捞鸥(俺牢)
...
void UB2UIGuildMercenary::SetMercenaryInfo(int32 InNPCID)
{
	...
	auto NPCData = GuildData.GetGuildMercenaryData(NPCID);			//磊扁 侩捍饭骇 版氰摹 殿殿 啊廉坷扁

	auto NPCMasterDataArr = GuildData.GetGuildMercenaryMasterData();		//侩捍 付胶磐 单捞磐 傈眉 啊廉坷扁
	auto NPCMercenariesData = GuildMasterData.GetMercenaries(NPCID);		//侩捍捞 公郊胶懦啊瘤绊乐绰瘤 荤侩咯何殿 啊廉坷扁
	b2network::B2mdGuildMercenaryMasteryPtr MercenaryMasteries = GuildMasterData.GetMercenaryMasteries(NPCID, NPCData->level);	//饭骇诀困茄版氰摹, 傈巩拳扁贱 构乐唱 啊廉坷扁

	b2network::B2mdGuildMercenaryDonationPtr MercenaryDonatioGold = GuildMasterData.GetMercenaryDonations(b2network::B2GuildMercenaryDonationType::GOLD);		//扁何榜靛沥焊
	b2network::B2mdGuildMercenaryDonationPtr MercenaryDonatioGem = GuildMasterData.GetMercenaryDonations(b2network::B2GuildMercenaryDonationType::GEM);		//扁何焊籍沥焊

	auto NPCMercenariesDataArr = NPCMasterDataArr.GetMercenariesArr();		//侩捍傈眉Array啊廉坷扁
...
void UB2UIGuildMercenary::DeliveryuildDonation(FB2ResponseGuildDonationPtr InGuildDonatio)
{
	...
	auto NPCData = GuildData.GetGuildMercenaryData(InGuildDonatio->mercenary->id);			//磊扁 侩捍饭骇 版氰摹 殿殿 啊廉坷扁]
	if (NPCData)
	{
...
void UB2UIGuildMercenary::UpgradeTick(float InDeltaTime)
{
	if (UpgradeTimeRemain > 0.f)
	{
		...
		int32 IncreaseExp = BeforeLevel == AfterLevel ? AfterExp - BeforeExp : (MercenaryMasteriesBeforeExp->max_exp - BeforeExp) + AfterExp;			//棵扼哎 剧狼 EXP
		int32 IncreaseTickExp = 0; //捞锅平狼 EXP
		int32 MaxExp = 0;
		float TickPercent = 0;
