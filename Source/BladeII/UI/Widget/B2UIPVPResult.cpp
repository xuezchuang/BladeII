...
{
	if (EStateAfterGradeUp::GRADE_ANIM_START == GradeUpState && ANIM_UP_01.IsValid()) // 등급업
	{
...
void UB2UIPVPResult::OnOpenAnimEnded()
{
	...
	if (GetGrade(GameModeEnum) < GetGrade(GameModeEnum, false)) // 등급업, 유지, 등급다운 결정
	{
		GradeUp = true;
	}
...
void UB2UIPVPResult::ShowRankingChanged(int32 DiffChangedRanking)
{
	if (DocSome.IsValid() && UIP_PVPRankingChangedPopup.IsValid())
	{
		...
		//UIP_PVPRankingChangedPopup->SetOtherRankingData(DocSome->GetOtherRankerInfo()); //임시
...
void UB2UIPVPResult::BindDocAuto()
{
		...
		if (BattleDoc && PvpDoc)
		{
			...
			if (TB_WinningBonus.IsValid())
			{
				TB_WinningBonus->SetVisibility(BattleDoc->GetTagMatchCurrContinuousWins() >=
					BladeIIGameImpl::GetClientDataStore().GetWinCountForBonus() ? ESlateVisibility::Visible : ESlateVisibility::Hidden);	//2연승이상일때 띄우기
	...
	case EB2GameMode::PVP_Team:
	{
		...
		if (TeamMatchDoc.IsValid())
		{
			...
			if (TB_WinningBonus.IsValid())
			{
				...
				// 1등급은 연승보너스 표기 안한다.																										 // 1등급은 연승보너스 표기 안한다.
	...
	if (TB_PVPRankPrevent.IsValid())
	{
		if (GetGrade(GameModeEnum, false) == GetGrade(GameModeEnum) &&						//등급과 별개수가 같으면 멘트 띄우기
	...
	// OV_DrawResult 추가될 수 있음. 그러면 무승부 상황에선 위에 OV_LoseResult 표시 안되게
...
bool UB2UIPVPResult::OnCheckAdditionalPoint_PvP()
{
	...
	if (PvPDoc->GetMatchPoint() == 0)
	{
		...
		UB2UIMsgPopupUseResources* MsgUI = UB2UIManager::GetInstance()->OpenMsgPopup<UB2UIMsgPopupUseResources>(EUIMsgPopup::UseGem,
			...
			FMsgPopupOnClick::CreateLambda([this]() { BTN_Restart->SetIsEnabled(false); }));// 취소버튼 눌렀을때 버튼 비활성화
	...
	check(TeamDoc)
		if (TeamDoc->GetTeamMatchPoint() == 0)
		{
			...
			UB2UIMsgPopupUseResources* MsgUI = UB2UIManager::GetInstance()->OpenMsgPopup<UB2UIMsgPopupUseResources>(EUIMsgPopup::UseGem,
				...
				FMsgPopupOnClick::CreateLambda([this]() {BTN_Restart->SetIsEnabled(false); }));// 취소버튼 눌렀을때 버튼 비활성화 // ClickNo Function
...
void UB2UIPVPResult::CheckAndOpenLevelUpPart()
{
	...
	// 레벨업 하는지 판단하는 부분은 총 2곳. 랭킹변경팝업이후 와, 랭킹변경이 없을경우 그 이전.
