...
void UB2UIQuestDialog::OpenRewardDialog(const class Quest& QuestRef, const TArray<b2network::B2RewardPtr>& ServerRewardInfo)
{
	...
	// MainQuest绰 辑滚俊辑 喊促弗 肯丰贸府 绝捞 官肺 促澜涅胶飘甫 捞绢焊郴霖促.
	// 咯扁鳖瘤 甸绢柯 QuestRef绰 角力肺 辑滚俊辑 肯丰灯促 窍歹扼档 EQS_PROGRESS 惑怕
	// 蝶扼辑 UI Flag父 EQS_COMPLETED肺 函版 秦 霖促.
...
void UB2UIQuestDialog::SetMissionDialogText(int32 InMissionID)
{
	if (MissionPtr Mission = MissionManager::GetInstance().GetMission(InMissionID))
	{
		...
		// Note : 扁夯 Title阑 八祸秦焊绊 绝促搁 Step阑 持绢辑 茄锅 歹 茫酒霖促.
...
void UB2UIQuestDialog::SetDialogText(int32 index)
{
	...
	PageCount = 0;
	// 其捞瘤逞辫 虐况靛 /page
	FString str = BladeIIGetLOCText(B2LOC_CAT_QUESTTEXT, FString::FromInt(index)).ToString();///烙矫 胶飘傅
...
{
	// 檬扁拳
	SetRewardButtonType(EQuestRewardButtonType::Max); // 焊惑 罐扁 滚瓢篮 檬扁拳 饶 酒贰俊辑 函版窍档废 窍磊.
...
void UB2UIQuestDialog::OnClickConfirm()
{
	...
	if (DialogType == EDialogType::Quest)
	{
		...
		case EQS_COMPLETED:
		{
			if (QuestType != MAIN_QUEST)
			{
				// MainQuest狼 版快 DivisionStat阑 碍力肺 EQS_COMPLETED肺 倒府哥
				// 辑滚俊 焊惑 夸没阑 且 鞘夸档 绝促.
		...
		case EQS_PROGRESS:
			if (QuestCommonInfo != nullptr)
			{
				...
				else if (IsInStageMode)	// Stage Result 惑炔俊辑 MainQuest 官肺啊扁
				{
	...
	else if (DialogType == EDialogType::Mission)
	{
		if (UB2UIDocSome* SomeDoc = UB2UIDocHelper::GetDocSome())
		{
			...
			if (MissionPtr Mission = MissionManager::GetInstance().GetMission(MissionID))
			{
				...
				auto MissionUI = UB2UIManager::GetInstance()->GetUI<UB2UIMission>(UIFName::Mission);		//固记 救俊辑 徘阑 官层具 窍绰版快
				if (MissionUI)
				{
