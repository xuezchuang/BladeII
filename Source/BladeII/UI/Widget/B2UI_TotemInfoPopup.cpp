...
{
	// 力访 肯丰登瘤 臼篮 配袍狼 版快 辑滚俊辑 霖 配袍 蔼捞 酒聪扼 皋牢Option 阑 烹秦 拌魂等 傈捧仿 父阑 钎扁茄促.
...
void UB2UI_TotemInfoPopup::OnDelaySellCallback()
{
	...
	else
	{
		...
		if (LobbyGM)
		{
			...
			if (SellItemPopup)
			{
				// PopupMain 俊 酒捞袍捞 酒囱 配袍捞 眠啊登菌阑 版快档 眠啊茄促.
				// 配袍 沥焊, 迫扁 喘范阑锭, 秒家 喘范阑锭
				SellItemPopup->PopupMain(CachedB2TotemInfo,
					FSellItemPopupOnClick::CreateLambda([this]() {
					Sell();
					this->OnClickBTN_Close(); // 迫酒摹奎栏聪 捞 芒档 摧绰促.
				}),
					FSellItemPopupOnClick::CreateLambda([this]() {})
					);
			}
