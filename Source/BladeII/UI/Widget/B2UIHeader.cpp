...
void UB2UIHeader::SetIsInDJLegacyHeroMgmt(bool bInDJLegacy, EHeroMgmtSubMode InSubMode)
{
	...
	if (bIsInDJLegacyHeroMgmt)
	{
		...
		ChangeLayoutByUIScene(DJLegacyHeroMgmtPageToUIScene(DJLegacyHeroMgmtSubMode)); // UIManager 矫胶袍狼 歹固 甘俏 纠 捞抚栏肺 持绢淋

		SetLobbyUIHeaderTitle(BladeIIGetLOCText(B2LOC_CAT_GENERAL, TEXT("LobbyHeroMgmt_Title"))); // 窍困 葛靛喊肺 技泼捞 肚 瞪 巴烙.
	}
	else
	{
		DJLegacyHeroMgmtSubMode = EHeroMgmtSubMode::EHMSM_End;
...
void UB2UIHeader::CacheAssets()
{
	...
	for (auto ModeOpenNoticeSlot : ModeOpenNoticeSlots)
	{
		if (ModeOpenNoticeSlot.IsValid())
		{
			...
			ModeOpenNoticeSlot->SetVisibility(ESlateVisibility::Collapsed); // 贸澜篮 救焊捞霸 汲沥
		}
	}
	GET_SLOT(UOverlay, Overlay_ModOpenNotice);
	Overlay_ModOpenNotice->SetVisibility(ESlateVisibility::Collapsed); // 贸澜篮 救焊捞霸 汲沥
...
void UB2UIHeader::BindDocAuto()
{
	...
	if (DocUICondition)
	{
		...
		//	ChangeLayoutByUIScene(DJLegacyHeroMgmtPageToUIScene(SubModeForLayout)); // UIManager 矫胶袍狼 歹固 甘俏 纠 捞抚栏肺 持绢淋
		//}
		//else
		//{ // DJLegacyHeroMgmt 俊辑档 捞霸 盔窍绰 蔼栏肺 唱棵 荐档 乐澜.
...
void UB2UIHeader::SetUserGuild()
{
	...
	if (BladeIIGameImpl::GetClientDataStore().GetUserGuildID())
	{
		...
		if (IMG_DefaultMark.IsValid()) {
			IMG_DefaultMark->SetVisibility(ESlateVisibility::Hidden); // GuildMark 尔 鞍篮 困摹.
		}
	}
	else
	{
		if (IMG_DefaultMark.IsValid()) {
			IMG_DefaultMark->SetVisibility(ESlateVisibility::HitTestInvisible); // GuildMark 尔 鞍篮 困摹.
		}
		if (UIP_GuildMark.IsValid()) {
...
{
	// 纠傈券瞪锭 沥焊盎脚秦霖促. 榜靛焊籍殿篮 平俊辑 秦林聪瘪 救秦拎档 登绰单. 弊成 茄锅歹 窍磊
...
void UB2UIHeader::ChangeLayoutByUIScene(EUIScene InScene)
{
	...
	// 纠官拆订 见变芭 秦力(见扁绰鉴埃 纠傈券老绢唱搁 庆歹啊 荤扼咙せ)
	if (m_bIsHideHeader)
	{
		HideHeader(false);
...
{
	if (bIgnoreBackBTN)
		//滚瓢 楷鸥矫俊 滚瓢捞 绝绰 Scene烙俊档 归洒胶配府肺 啊绰 巴阑 规瘤
		return;
