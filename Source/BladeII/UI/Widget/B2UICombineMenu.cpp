...
{
	int32 SvrPCype = 0; // 葛电 努贰胶
	int32 SvrEquipCategory = 0;	// 傈眉 辆幅 酒捞袍
...
void UB2UICombineMenu::OnClickBTN_GoogleMission()
{
	...
	{
		B2P_ShowAchivementView();	// 备臂 诀利 拳搁栏肺~
	}
	else
	{
		UB2UIManager* UIMgrInst = UB2UIManager::GetInstance();
		if (UIMgrInst)
		{
			UIMgrInst->OpenMsgPopup<UB2UIMsgPopupSimple>(EUIMsgPopup::Simple,
				...
				BladeIIGetLOCText(B2LOC_CAT_GENERAL, TEXT("Google_MissionFeatured")), // 措帆 捞繁 郴侩狼 版绊 巩磊啊 甸绢啊摆瘤..
				0.f,
				true,
				true,
