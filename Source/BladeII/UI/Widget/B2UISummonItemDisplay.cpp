...
void UB2UISummonItemDisplay::OnOpen(bool RightNow /*= false*/)
{
	...
	EnterShopTicket = EnterShopClass<int32>::GetInstance().Subscribe(			//康去离 葛肋扼辑 惑痢艾促啊 第肺啊扁沁阑矫 坷搁XX 抗寇贸府
...
void UB2UISummonItemDisplay::OnClickBTN_OpenTouch()
{
	if (bCardOpenTouch)
	{
		if (OpenCardNumber == 0)
		{
			...
			{
				for (int32 Idx = 0; Idx < UIP_SummonedItems.Num(); ++Idx)			//快急 滴锅父 倒档废 汲沥
				{
...
void UB2UISummonItemDisplay::OnClickBTN_SummonOneMore()
{
	...
	if (DocStore && DocUser)
	{
		...
		if (Data)
		{
			// 楷加惶扁捞绊 10雀惶扁鼻捞 乐促搁 10雀惶扁鼻阑 快急栏肺 家葛
			...
			if (IsTenTicket) // 10雀 惶扁鼻老 版快 抗寇贸府
...
{ // bCardResult 啊 奴芭 窍唱 焊咯临瘤 累篮 芭 咯矾俺 焊咯临 瘤 弊扒 淀
...
void UB2UISummonItemDisplay::ChangeSummonItemCardMatineeActorToFX(const TArray<FB2Item>& Items, ALevelSequenceActor * Matinee)
{
	...
	//			SummonBoxActor = Cast<ASkeletalMeshActor>(ThisGroupInst->GetGroupActor());					// 惶腮 墨靛 啊廉坷绊


	//		if (ThisGroupInst->Group->GroupName == TEXT("Loot_Card10BG"))
	//			SummonBoxActorBG = Cast<ASkeletalMeshActor>(ThisGroupInst->GetGroupActor());				//措扁吝牢墨靛档 掘绢坷绊

	//		if (ThisGroupInst->Group->GroupName == TEXT("Cam01"))
	//			CameraInfo = Cast<ACameraActor>(ThisGroupInst->GetGroupActor());			//墨皋扼啊 阿档啊 促福促 ばば 墨皋扼档 掘绢坷磊...
	//	}
	//}
...
void UB2UISummonItemDisplay::UpdateLocationSummonItemUIs()
{
	...
	//					// 困摹 炼例阑 困秦 CanvasPanelSlot 俊 硅摹秦具 窃.
	...
	//						// 浇吩 荤捞令父怒 焊沥. Size to Content 沁阑 矫 GetDesiredSize 肺.. 
	//						FVector2D ItemSlotSize = ThisSlot->GetAutoSize() ? UIP_SummonedItems[Idx]->GetDesiredSize() : ThisSlot->GetSize();
	//						// 胶纳老捞 眠啊肺 乐绢辑.. 辟单 GetDesiredSize 搁 Scale 捞 馆康登绢 乐阑 淀..?
	...
	//						ThisSlot->SetAnchors(FAnchors(0.f, 0.f)); // 捞繁 扒 柳累俊 嘎苗初绰 吧肺..
...
void UB2UISummonItemDisplay::UpdateButtonState(int32 CardNumber)
{
	...
	if (CP_Confirm.IsValid() && TB_Confirm.IsValid())
	{
		...
		{
			if (OpenCardMaxNumber == 10 && CardNumber >= 0 && !bCardResult) // 10厘楼府 贸澜俊
				VisibilityButtonState = true;
		}
...
// Tutorial - 300_5 家券 酒捞袍 Click矫 Detail沥焊啊 哆绰巴阑 规瘤窍扁 困茄 窍靛内爹
