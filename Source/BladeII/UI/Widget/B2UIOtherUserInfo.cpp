...
void UB2UIOtherUserInfo::TotalUpdate(EPCClass PCClass)
{
	...
	LobbyHeroFindAccountClass<EPCClass>::GetInstance().Signal(PCClass);			//某腐磐皋浆 官层林扁

	UpdateCharName(PCClass);								//滚瓢俊 流诀 捞抚甸
	auto& UserInfo = BladeIIGameImpl::GetClientDataStore().GetOtherUserInfo();

	Lobby_CharStatDetailBP->UpdateStats(PCClass, true);	//惑技沥焊

	SetBasestats(UserInfo, PCClass);						//扁夯傍规 饭骇殿
	SetUserNickName(FText::FromString(UserInfo.GetUserNickName()));	//葱匙烙
	ItemSet(UserInfo, PCClass);								//酒捞袍技泼
...
{
	if (SelectPCClass == EPCClass::EPC_Gladiator) return; //[@AKI, 171122][B2BUG-836]鸥 蜡历狼 某腐磐 惑技焊扁俊辑 某腐磐 殿厘 捞棋飘 馆汗秦辑 唱坷绰 坷幅 荐沥
...
{
	if (SelectPCClass == EPCClass::EPC_Assassin) return; //[@AKI, 171122][B2BUG-836]鸥 蜡历狼 某腐磐 惑技焊扁俊辑 某腐磐 殿厘 捞棋飘 馆汗秦辑 唱坷绰 坷幅 荐沥
...
{
	if (SelectPCClass == EPCClass::EPC_Wizard) return;//[@AKI, 171122][B2BUG-836]鸥 蜡历狼 某腐磐 惑技焊扁俊辑 某腐磐 殿厘 捞棋飘 馆汗秦辑 唱坷绰 坷幅 荐沥
...
{
	if (SelectPCClass == EPCClass::EPC_Fighter) return;//[@AKI, 171122][B2BUG-836]鸥 蜡历狼 某腐磐 惑技焊扁俊辑 某腐磐 殿厘 捞棋飘 馆汗秦辑 唱坷绰 坷幅 荐沥
