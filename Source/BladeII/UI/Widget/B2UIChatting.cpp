...
void UB2UIChatting::Init()
{
	...
	//SetChatCategoryTab(EB2UIChatCategory::UICHTY_All); // 咯扁辑 窍瘤 臼绰促. ScrollBox 啊 肯傈洒 檬扁拳 等 惑怕啊 酒丛..

	BladeIIGameImpl::GetChatStore().RequestGetChannelNum();

	UpdateControlInfo(); // RequestGetChannelNum 俊 狼秦 览翠捞 坷搁 促矫 技泼登绢具.

	UB2UIManager* UIMgrInst = UB2UIManager::GetInstance();
	if (UIMgrInst)
	{
		UB2UIMain* UIMain = UIMgrInst->GetMainWidget();
		// 盲泼 芒 绝阑 锭俊 钎矫窍绰 茄临 盲泼捞 凯妨 乐促搁 官肺 摧绰促. 
...
void UB2UIChatting::OnOpen(bool RightNow)
{
	...
	//SetChatCategoryTab(EB2UIChatCategory::UICHTY_All); // 捞傈俊 盲泼 芒阑 凯菌阑 锭狼 徘 急琶 惑怕 某教捞 鞘夸且 荐 乐澜..
	//UpdateGuildBadge();
...
void UB2UIChatting::UpdateChatMessageAll()
{
	...
	// GetChatMessages 俊辑 掘篮 扒 货 皋矫瘤啊 菊栏肺 沥纺捞 登绢 乐澜. 胶农费 冠胶俊绰 坷贰等 皋矫瘤何磐 持绢具 窃. 第俊辑何磐.
	for (int32 MI = AllMessages.Num() - 1; MI >= 0; --MI)
	{
		...
		if (AddedRowSlot)
		{
			// 趣矫 Margin 汲沥 鞍篮 芭 鞘夸窍搁 AddedRowSlot 阑 啊瘤绊..
...
void UB2UIChatting::UpdateChatScrollEnd(bool bForce)
{
	...
	if (bForce && (bBlockScrollBox == false))
		SB_MessagePanel->ScrollToEnd(); // 盖 酒贰俊 弥脚 皋矫瘤啊 乐促.
}
...
void UB2UIChatting::SetChatCategoryTab(EB2UIChatCategory InUIChatCategory)
{
	...
	// 辨靛 备泅 傈鳖瘤 辨靛 盲泼 徘篮 见变促. 荤侩磊啊 辨靛 啊涝登绢 乐绰 版快父 荤侩.
...
class UB2UIChatMessageBase* UB2UIChatting::GetUIMessageSlot(const FB2ChatMessage& InMessageInfo)
{
	...
	//	// SB_MessagePanel 狼 胶农费 勤甸傅阑 困茄 殿废. ThisChatMsg 狼 滚瓢捞 涝仿阑 啊肺盲瘤 臼绊 逞败林绢辑 流立 勤甸傅 窍档废.
...
// 盲泼芒阑 劝侩茄 能贾目盖靛.. 俊鼓饭捞磐 券版俊辑 荤侩阑 困秦.
bool UB2UIChatting::CheckAndExecuteConsoleCommandFromChatInput(const FString& ChatEditMsg)
{
	...
	//4Finger尔 悼老茄 累悼阑 窍绰 内靛
	/*
	static const FString SYS_MSG_PREFIX(TEXT("//sys"));

	FString CmdOnly = ChatEditMsg;
	if (InPC && CmdOnly.RemoveFromStart(SYS_MSG_PREFIX)) // 眠啊肺 函荐 窍唱甫 歹 滴带瘤..
	{
		CmdOnly.RemoveFromStart(TEXT(" ")); // 焊烹 prefix 第俊 傍归阑 滴霸 瞪 抛聪 乐栏搁 力芭. 绝促绊 秦档 弊措肺 牢侥窍档废 窍绊.

		InPC->ConsoleCommand(CmdOnly);
		return true;
	}
	/*/
	//[@AKI, 180328][TO-DO] Android Build俊辑 Casting锭巩俊 促矫 犬牢 秦毫具 且巴
...
void UB2UIChatting::OnClickBtnEnterChatMsg()
{
	if (EDIT_ChatMsg.IsValid())
	{
		...
		EdittedMsg = ClampTextLength(EdittedMsg, FB2ChatStore::MessageLengthLimit); // 辨捞绰 拱沸 俊叼飘 档吝俊 肋妨具 窃. 弥辆 八荤老 挥.

		if (!CheckAndExecuteConsoleCommandFromChatInput(EdittedMsg.ToString()) &&
			EdittedMsg.ToString().Len() > 0 && 
			!BladeIIGameImpl::GetChatStore().IsPossibleSpamMessage(EdittedMsg)) // 档硅 眉农
		{
			// @TODO Chat 厚加绢 鞘磐
	...
	if (UIMgrInst)
	{
		...
		if (InputPopup)
		{
			InputPopup->AddHandler(EUIMsgPopupButton::Positive, FMsgPopupOnClick::CreateLambda([this, InputPopup](){
				...
				{ // 涝仿 裹困啊 肋给登菌促搁 促矫 捞 皋矫瘤 冠胶甫 咯绰 救郴 皋矫瘤. 泅 备炼惑 摧瘤 臼霸绰 给且 淀.
					UB2UIManager* UIMgrInst2 = UB2UIManager::GetInstance();
					if (UIMgrInst2)
					{
...
{
	UpdateControlInfo(); // 绢瞒乔 ChatStore 俊 甸绢啊 乐绰 蔼阑 镜芭.
}
