...
void FB2ModPlayerInfo::SetModPlayerInfo(const FB2FindAccountInfoPtr& InPlayerInfo)
{
	...
	{ 
		...
		// InPlayerInfo->wing_options 吝俊辑 捞锅 某腐磐 巴父 哗 咳. 酒流 茄锅档 GetWing 阑 窍瘤 臼篮 某腐磐扼搁 可记 单捞磐啊 救 棵 荐 乐促.
		...
		ThisModCharInfo.WingContainer.SetHasWingByWingData(); // bHasWing 阑 蝶肺 辑滚俊辑 罐酒具 且 芭绰 鞍篮单 老窜 绝栏聪..
	}
	...
	for (auto RelicItem : InPlayerInfo->relics) // 泅犁 坷锹等 蜡拱狼 沥焊.
	{
	...
	for (b2network::B2AetherServerInfoPtr EtherItem : InPlayerInfo->mount_aethers) // 俊抛福 沥焊
	{
	...
	for (auto UnityItem : InPlayerInfo->unity_skills) // 泅犁 碍拳等 搬加胶懦 沥焊
	{
		for (auto& Character : Characters)
		{
...
float FB2ModPlayerInfo::GetCombatStatusValue(EPCClass InPCClass, TArray<FB2Item>* InItemArray/* = NULL*/) const
{
	...
	// 厘馒 厘厚甫 蝶肺 瘤沥且 荐 乐促. Local 酒囱 ModPlayerInfo 俊辑 弊凡 老捞 乐阑瘤绰 葛福摆瘤父
