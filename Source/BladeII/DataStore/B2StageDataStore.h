...
struct FStageInfoGameplayData
{
	FStageInfoGameplayData(const b2network::B2mdStageInfoPtr& InServerinfoPtr)
	{
		...
		if (InServerinfoPtr)
		{
			...
			ClientStageId = InServerinfoPtr->map_id;	// 이게 클라이언트에서 사용하는 stageId이다. (서버에서는 map_id라고 넘어옴)
...
public:
	...
	FServerStageID		GetLastClearServerStageIdByLastPlayedStageDifficulty();	// 마지막 클리어한 난의도의 마지막 스테이지 아이디를 반환
	FServerStageID		GetNextClearServerStageIdByLastPlayedStageDifficulty();	// 마지막 클리어한 난의도의 마지막 스테이지 아이디+ 1을 반환 

	FServerStageID		GetNewLastClearServerStageIdByLastPlayedStageDifficulty(int32 InChapter);	//6챕터이후 마지막 클리어한 난도의 마지막 스테이지 +1, 더 깰게 없으면 마지막 클리어한 스테이지
	
	EStageDifficulty	GetMaxOpenedStageDifficulty();							//기존 1~5난이도 주는 함수
	EStageDifficulty	GetMaxOpenedNewStageDifficulty(int32 InChapter);		//6~이후 난이도 주는 함수
	bool				GetNewChapterOpenState(int32 InChapter);				//6부터 오픈이 된건지 안된건지
	...
	int32				LoadOpenAct();
	bool				LoadCBTClear();
	//[@AKI, 170908] B2BUG-704 관련 추가 함. 704 수정을 위해서는 LoadClearAct와 LoadOpenAct를 수정해야 함
