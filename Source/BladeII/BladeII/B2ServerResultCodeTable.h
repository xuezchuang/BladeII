...
{	
	...
	//donaiton
	const int FAILURE_NOT_EXIST_DONATION_STATUS							   = 6800; // 기부 상태 정보가 존재하지 않을 경우
	const int	FAILURE_DUPLICATED_DONATION_STATUS						   = 6801; //	중복된 기부 상태 정보가 존재 할 경우
	const int	FAILURE_NOT_FREE_DONATION_STATE							   = 6802; //	무료기부 시 무료기부 횟수를 모두 소진했거나 묶음기부의 상태요청이 존재할 경우
	const int	FAILURE_NOT_MATCHED_LAST_DONATION_KEY_VALUE				   = 6803; //`한번 더` 기부 검증용 키값이 유효하지 않을 경우
	const int	FAILURE_UPDATE_DAILY_FREE_COUNT							   = 6804; //일일 무료 기부 개수 업데이트가 실패했을 경우
	const int	FAILURE_UPDATE_DONATION_POINT							   = 6805; //	기부 포인트 업데이트가 실패했을 경우
	const int	FAILURE_UPDATE_LAST_DONATION_KEY_VALUE					   = 6806; //`한번 더` 기부 검증용 키 업데이트가 실패했을 경우
	const int	FAILURE_INVALID_DONATION_ID								   = 6807; //기부 이벤트 아이디가 유효하지 않은 경우
	const int	FAILURE_ALREADY_REWARDED_DONATION_POINT_REWARD		   	   = 6808; //	이미 수령한 기부 포인트 보상을 다시 요청한 경우
	const int	FAILURE_UPDATE_DONATION_POINT_REWARD_STATE				   = 6809; //기부 포인트 보상 상태 정보 업데이트가 실패했을 경우
	const int	FAILURE_NOT_ENOUGH_POINT_RECEIVE_REWARD					   = 6810; //	기부 포인트 별 보상 받기 시 포인트가 부족할 경우
	const int	FAILURE_DUPLICATE_DONATION_EVENT_IN_PROGRESS			   = 6813; //	진행중인 기부 이벤트가 2개 이상일 경우
	const int	FAILURE_YET_REMAIN_FREE_DONATION_COUNT					   = 6814; //유료기부 시 무료기부 횟수가 남아 있을 경우
	const int	FAILURE_THIS_DONATION_EVENT_ALREADY_ENDED				   = 6815; //기부 시 기부 이벤트가 이미 종료된 경우
	const int	FAILURE_NOT_YET_NEXT_DONATION_TIME						   = 6822;				// 기부 Request 1초 제한
	//mission
