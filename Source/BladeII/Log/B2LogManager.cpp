...
{
	std::transform(_name.cbegin(), _name.cend(), m_namelower.begin(), ::tolower); // 소문자로 전환
}
...
void B2LogFilterManager::AddFilter(const std::string& _filterName)
{
	...
	std::transform(_filterName.cbegin(), _filterName.cend(), _filterNameLower.begin(), ::tolower); // 소문자로 전환
	//Value Check
...
std::shared_ptr<B2LogFilterManager> B2LogFilterManager::GetInstance()
{
	...
	BII_CHECK(m_pInstance);//그럴 일은 거의 없겠지만 혹시 필터 때문에 죽을수도 있어서 체크함
	return m_pInstance;
}
