...
float ABladeIINetPlayer::TakeDamage(float Damage, struct FDamageEvent const& DamageEvent, class AController* EventInstigator, class AActor* DamageCauser)
{
	...
	//		// ActualDamage 甫 0栏肺 绊沥矫懦 荐 乐绰 夸家甸

	//		const bool IsValidEnemy			= GetAttacker(DamageCauser) != nullptr;
	//		const bool HasSuperArmor		= IsBuffActive(EBuffType::Buff_Unbeatable);
	//		const bool InvinsibleVariable	= bInvincible; // 公利滚橇 + @ (漂沥 葛靛狼 楷免 惑炔)栏肺 公利惑怕狼 醚褒俺充

	//		// 1. 傍拜磊甫 茫阑 荐 绝阑版快  
	//		// 2. 公利 滚橇啊 绝绰单 Invinsible牢 版快(狼档茄 惑炔老 荐 乐栏唱 犬伏捞 撤栏骨肺 老窜 Log焊晨)
...
void ABladeIINetPlayer::OnDeath(float KillingDamage, struct FDamageEvent const& DamageEvent, class APawn* InstigatingPawn, class AActor* DamageCauser)
{
	...
	//	break;
	//}
	//case EB2GameMode::Control: //牧飘费篮 鞘夸矫 蝶肺 备泅
	//default: //[@AKI, 170629] Raid, Tag, Control 力寇窍绊 泅犁 BladeIINetPlayer甫 荤侩窍绰 版快啊 绝绰 吧肺 舅绊 乐澜
	//	break;
	//}
}
...
{
	// !!!吝夸!!!!! this == Victim
	// 傍拜磊档 乔秦磊档 Authority老 版快绰 被捞 broadcasting 且 鞘夸 绝澜
...
void ABladeIINetPlayer::SetAttackState(EAttackState InAttackState)
{
	...
	// ComboEnd俊 狼秦辑 沥惑利栏肺 惑怕啊 辆丰登绰 纳捞胶俊辑绰 ECS_None 惑怕肺 官层 拎具茄促
...
void ABladeIINetPlayer::OnEnterAnimState(const FName& NewState)
{
	...
	//{
	//	/*
	//	// B2BUG-1616 - Packet 弥利拳.
	//	// Animation Simulate 悼扁拳 锭巩俊 持篮 扁瓷捞唱 静瘤 臼栏骨肺 菩哦 价脚 X
...
void ABladeIINetPlayer::SendAnimStateIndex(int32 NewStateIndex)
{
	...
	//	// 局丛教农沥焊 struct FNetSyncAnimInfo 葛剧
	//	
...
//
//	// 痢飞傈俊辑 QTE倾啊绰 龋胶飘啊窍绊, QTE林眉档 龋胶飘 倾啊甫 罐绊 QTE柳涝茄促.
//	// 捞 笛 捞寇狼 敲饭捞绢绰 QTE敲饭捞绢啊 QTE柳涝饶俊 捞悼菩哦阑 罐阑荐档 乐澜.
//	// ex) 敲饭捞绢1 扁扁 QTE夸没 - 龋胶飘 倾啊 - 唱赣瘤 葛电 扁扁 敲饭捞绢1 QTE角青
//	// - 敲饭捞绢1 扁扁 匙飘况农 瘤楷栏肺 QTE角青救窍备 捞悼菩哦 傈价饶 QTE角青 
//	// - 唱赣瘤敲饭捞绢 扁扁 敲饭捞绢1 QTE角青饶 捞悼棺 肺抛捞记 利侩
//	if (IsInQTEState())
//		return;
//
...
//
//	// 府葛飘绰 傈规 喉肺欧 焊沥侩 函荐档 盲澄烹秦 罐篮吧肺 悸泼
...
// 	//SyncLocationAndRotation(pos, rot);
// 
// 	// ABladeIIPlayer::StartGuard() 捞侩窍瘤 臼绊 蔼父 汲沥秦霖促.
...
void ABladeIINetPlayer::RemoteSkillAttack(uint8 state, uint8 anim_index, FVector pos, FRotator rot)
{
	...
	// 胶懦->胶懦 胶抛捞飘傈券绝捞 楷搬登绰 纳捞胶 积扁搁 夸芭 林籍钱扁
...
void ABladeIINetPlayer::RemoteStartTag(ETagAttackMode state, EPCClass CharClass, FVector pos, FRotator rot)
{
	...
	// 局丛胶抛捞飘 悸泼. 胶懦胶抛捞飘俊辑 胶抛捞飘 函版绝捞 敲贰弊父 函版登绰 纳捞胶啊 乐促.
	// 惑措率 肺拿敲饭捞绢俊辑 SendAnimStateIndex甫 给谨林聪 敲贰弊扼档 悸泼秦淋.
	// 沥惑利栏肺 胶抛捞飘函版栏肺牢秦 怕弊(胶懦 胶抛捞飘) AnimStateIndex甫 罐栏搁 酒贰沥焊绰 丹绢镜芭烙
...
{
	////if (EtherOwnerNetId == GetNetId() && GetEtherContainer())	// EtherOwner啊 this扼搁 ( Authority/simulated 惑包绝澜 )
	//{
	//	if (UB2EtherSetEffect* EtherObject = GetEtherContainer()->GetEtherSetEffect(EtherSetID))
	//	{
	//		// ex - Offense : Thunder Ether [ EtherCauser == LocalPawn(Authority), EtherOwner == this(Simulated) ] 啊 等促.
	//		// ex - Defense : Burning Ether [ EtherCauser == this(Simulated), EtherOwner == LocalPawn(Authority) ] 啊 等促.
	//		 
...
{// 霸烙 饭骇 救俊辑 夯 某腐磐甫 侥喊 啊瓷窍霸 且 ID 啊 凳. NetID 甫 馆康茄促.
	// 荤侩磊啊 炼累窍绰 肺拿 敲饭捞绢 某腐磐扼搁 NetId 甫 馆康窍瘤 臼绰单 NetId 啊 绝绰 惑炔俊辑档 侥喊捞 鞘夸茄 版快啊 乐绢辑.
...
{
	return IsLocalPlayer() || (NetId != 0); // 盔拜 某腐磐绰 酒流 NetId 甫 何咯罐瘤 给茄 惑炔俊辑绰 GameEntryID 甫 静搁 救凳.
}
