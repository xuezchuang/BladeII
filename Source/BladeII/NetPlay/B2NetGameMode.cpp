...
void AB2MatchGameModeBase::ResponseSendRoomPlayerClass(TArray<FB2NetMatchPeerInfo> const& PeerInfos)
{
	...
	// 冯捞 悸泼等 促澜俊 秦寸 菩哦捞 傈崔登绢具 沥惑悼累捞瘤父..
	// 版快(叼官捞胶 惑炔 殿殿)俊 蝶扼辑 冯捞 悸泼登扁傈俊 龋免登绰 版快档 惯积窍扁俊 抗寇贸府
...
void AB2MatchGameModeBase::ResponseAckPrerequisiteToStart(uint32 NetId)
{
	...
	//// PEER_SUPER老 版快 霖厚啊 促登菌栏搁 辑滚肺 菩哦 傈价
	//packet::ByteStream	payload;
...
{
	// 敲阀汽 Pause 惑怕老锭 甸绢坷绰 窃荐
	// 捞锭 老馆利牢 版快 盲澄 辆丰甫 矫糯.
	ShutdownChannelIfStillExist();
}
...
void AB2MatchGameModeBase::PostRemotePlayerInfoData()
{
	...
	// 冯悸泼傈俊 龋胶飘肺何磐 菩哦阑 固府 罐疽栏搁 蜡历甸俊霸 郴 沥焊档 傈崔
...
void AB2MatchGameModeBase::ApplyReservedDecodePacket()
{
	...
	// 汗荤窍备 厚框. 历 酒贰 叼内靛救俊辑 皋技瘤 焊郴绰 纳捞胶档 乐绰芭鞍促.
	// 弊成 静搁 馆汗巩救俊辑 硅凯夸家 眠啊瞪荐乐澜.
...
void AB2MatchGameModeBase::RequestDeclineInvitation(EB2GameMode GameModeType)
{
	...
	//		// Request率篮 Notify啊 坷瘤臼扁锭巩俊 流立 龋免秦霖促
	//		const FString MyNickName = BladeIIGameImpl::GetLocalCharacterData().GetUserNickName();
...
bool AB2MatchGameModeBase::CanReceiveGameInvite(const FString& Inviter, EB2GameMode GameModeType) const
{
	...
	//	// 笛促 檬措罐瘤 臼篮 惑怕俊辑父 檬措甫 罐阑 荐 乐澜 ex) 饭捞靛 檬措吝 PVP檬措 阂啊
...
//
//	// B2NetGameMode霸烙狼 版快 官肺 概莫阑 龋免秦霖促.
//	SetMatchState(NetMatchState::RequestMatchMaking);
//
//	//World狼 BeginPlay矫痢捞 第俊辑 矫累凳俊 蝶扼
//	//颇萍努 积己捞 瘤楷登绰 巩力啊 乐促.
//	//泅 B2NetGameMode狼 备炼客 包访捞 乐绢
//	//酒贰贸烦 埃窜洒 贸府窍唱, 促弗 咀磐俊档 巩力啊 乐促搁 备炼甫 官层具且 淀.
...
TArray<EPCClass> AB2NetGameMode::GetAllNetGameMatchedPCClasses()
{
	...
	//// 怕弊啊 绝绰 霸烙葛靛扼搁 固府 肺爹窍瘤 臼酒档 老窜 积己 矫 喉肺欧捞 吧府瘤绰 臼栏唱 搬惫 Pre-render 甫 烹秦 嘉捞歹 某教阑 窍妨搁 鞘夸窃. 弊府绊 Skill Anim 档 蝶肺 盒府登绊..
	//// 惑措 沥焊啊 Preload 矫痢 捞饶俊 甸绢坷芭唱, 怕弊啊 乐芭唱 窍搁 捞吧 override 秦具 窃.
...
//#if WITH_EDITOR
//	if (GIsEditor)
//	{ // 俊叼磐俊急 葛电 skill 局聪皋捞记阑 肺爹窍档废 窍绰 惑困 努贰胶 扁瓷阑 荤侩
...
ABladeIIPlayer* AB2NetGameMode::SpawnRemotePlayer(class UWorld* SpawnWorld, EPCClass InClass, const FTransform& SpawnTransform, FActorSpawnParameters& SpawnParameters)
{
	...
	//FinalSpawnParameter.bDeferPostSpawnInitialize = true; // bDeferConstruction 包访 贸府绰 观俊辑 窍瘤父 bDeferPostSpawnInitialize 绰 咯扁急 公炼扒 荤侩秦具. 
	...
	//SpawnWorld->ManualPostSpawnInitialize(SpawnedActor, SpawnTransform, FinalSpawnParameter); // bDeferPostSpawnInitialize 俊 蝶弗 贸府.
	//return SpawnedActor;
	return NULL;
}
...
void AB2NetGameMode::ReceiveAnimStateIndex(uint32 RemoteNetId, FNetSyncAnimInfo& NetSyncAnimInfo)
{
	...
	//
	////葛电 菩哦阑 促 历厘 且 荐 绝栏骨肺 沥秦柳 肮荐 捞惑捞搁 昏力
